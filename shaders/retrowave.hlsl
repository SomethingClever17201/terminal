...
float grid(float2 uv, float battery)
{
    ...
    uv = abs(frac(uv) - 0.5);
 	float2 lines = smoothstep(size, float2(0.0,0), uv);
 	lines += smoothstep(size * 5.0, float2(0.0,0), uv) * 0.4 * battery;
...
{
    float2 uv = (2.0 * pos.xy - iResolution.xy)/iResolution.y;
    uv.y = 1 - uv.y - 1;
    float battery = 1.0;
    ...
    //if (abs(uv.x) < (9.0 / 16.0))
    {
        ...
        else
        {
            ...
            // cloud
            float2 cloudUV = uv;
            cloudUV.x = ((cloudUV.x + Time * 0.1) % 4.0) - 2.0;
        ...
        return float4(col.x, col.y, col.z,1.0) + shaderTexture.Sample(samplerState, tex);
    }
